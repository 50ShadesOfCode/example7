...
public class Books {
    ...
    static Preparate readBook(Scanner fin ) throws IOException {
        return Preparate.nextRead( fin, bookOut )
                ? Preparate.read( fin, bookOut ) : null;
    }
    ...
    static boolean deleteFile( String[] args ) 
        ...
        try ( Index idx = Index.load( idxname ); 
            ...
            while (( pos = fileBak.getFilePointer()) < fileBak.length() ) {
                Preparate preparate = (Preparate)
                Buffer.readObject( fileBak, pos, wasZipped );
                if ( Arrays.binarySearch(poss, pos) < 0 ) { // if not found in deleted
                    long ptr = Buffer.writeObject( file, preparate, wasZipped[0] );
                    idx.put(preparate, ptr );
                }
             }
         }
         return true;
    ...
    static void appendFile( Boolean zipped ) 
        ...
        try ( Index idx = Index.load( idxname ); 
            ...
            for(;;) {
                Preparate preparate = readBook( fin );
                if ( preparate == null )
                    break;
                idx.test(preparate);
                long pos = Buffer.writeObject( raf, preparate, zipped );
                idx.put(preparate, pos );
            }
        }
    }
    ...
    private static void printRecord( RandomAccessFile raf, long pos ) 
        ...
        boolean[] wasZipped = new boolean[] {false};
        Preparate preparate = (Preparate) Buffer.readObject( raf, pos, wasZipped );
        if ( wasZipped[0] == true ) {
