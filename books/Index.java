...
public class Index implements Serializable, Closeable {
	...
	IndexOne2One apNumber;
	IndexOne2N dateFroms;
	IndexOne2N names;

	IndexOne2N dateTos;

	public void test(Preparate preparate) throws KeyNotUniqueException {
		assert (preparate != null);
		//check requirements for unique keys
		if (apNumber.contains(String.valueOf(preparate.apNumber))) {
			throw new KeyNotUniqueException(String.valueOf(preparate.apNumber));
		}
	}

	public void put(Preparate preparate, long value) throws KeyNotUniqueException {
		test(preparate);
		apNumber.put(String.valueOf(preparate.apNumber), value);
		dateFroms.put(preparate.dateFrom, value);
		dateTos.put(preparate.dateTo, value);
		names.put(preparate.name, value);
	}

	public Index() {
		apNumber = new IndexOne2One();
		dateFroms = new IndexOne2N();
		names = new IndexOne2N();
		dateTos = new IndexOne2N();
	}
